...
contextBridge.exposeInMainWorld('imageStudio', {
  ...
  saveResourceCategories: (categories: ResourceCategory[]) => ipcRenderer.invoke('resource:save-categories', categories),
  updateResource: (assetId: string, update: { name?: string; resourceCategoryId?: string; resourceProcessingPrompt?: string }) => ipcRenderer.invoke('resource:update', assetId, update),
  addGenerationToResource: (recordId: string, categoryId: string) => ipcRenderer.invoke('resource:add-generation', recordId, categoryId),
  pickResourceImages: (categoryId: string) => ipcRenderer.invoke('resource:pick-images', categoryId),
  ...
  pasteImage: () => ipcRenderer.invoke('asset:paste-image'),
  ...
  retryGeneration: (recordId: string) => ipcRenderer.invoke('generation:retry', recordId),
  setGenerationReview: (recordId: string, reviewStatus: 'qualified' | 'rejected' | null) => ipcRenderer.invoke('generation:set-review', recordId, reviewStatus),
  generateProductMainPrompts: (assetId: string, ratioLabel: string) => ipcRenderer.invoke('text:generate-product-main-prompts', assetId, ratioLabel),
  clearProductMainRecords: () => ipcRenderer.invoke('product-main:clear-records'),
  ...
  chooseOutputDirectory: () => ipcRenderer.invoke('output:choose-directory'),
